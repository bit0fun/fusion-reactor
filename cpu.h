...
void lw( uint32_t* data_mem, dataseg_info dseg_i, insn_info* insn, uint32_t (*registers)[32] ){ 
  uint32_t idx = (( (*registers)[insn->rsa])) + (((insn->imm & 0x00003fff) ))/4;// - dseg_i.start)/4 ;
...
int execute( uint32_t* pc, uint32_t** tseg, textseg_info* tseg_i, uint32_t** data_mem, dataseg_info* dseg_i, uint32_t  (*registers)[32], insn_info* insn_i ){
